...
var (
	...
	excludedFiles = map[string]bool{
		...
		"github.com/mwitkow/go-proto-validators/validator.proto":   true,
		"github.com/gomeet/go-proto-gomeetfaker/gomeetfaker.proto": true,
		"github.com/gogo/protobuf/gogoproto/gogo.proto":            true,
	}
...
func processFile(inFile *descriptor.FileDescriptorProto, elmPrefix string, jsonNameType JsonNameType) (*plugin.CodeGeneratorResponse_File, error) {
	...
	for _, inEnum := range inFile.GetEnumType() {
		...
		err = fg.GenerateEnumFromStringFunction("", inEnum)
		if err != nil {
			return nil, err
		}

		err = fg.GenerateEnumToStringFunction("", inEnum)
		if err != nil {
			return nil, err
		}
...
func (fg *FileGenerator) GenerateComments(inFile *descriptor.FileDescriptorProto) {
	...
	fg.P("-- https://github.com/gomeet/elm-protobuf")
...
func (fg *FileGenerator) GenerateEverything(inFile *descriptor.FileDescriptorProto, prefix string, inMessage *descriptor.DescriptorProto, jsonNameType JsonNameType) error {
	...
	for _, inEnum := range inMessage.GetEnumType() {
		...
		err = fg.GenerateEnumFromStringFunction(newPrefix, inEnum)
		if err != nil {
			return err
		}

		err = fg.GenerateEnumToStringFunction(newPrefix, inEnum)
		if err != nil {
			return err
		}
	}
